		...
		if(m_FavLookup.m_FavoriteIndex != -1)
		{
			if(m_FavLookup.m_HostLookup.m_Job.Result() == 0)
			{
				...
				if(pEntry)
				{
					...
					if(pEntry->m_State != FAVSTATE_HOST)
					{
						...
						dbg_msg("serverbrowser", "fav aquired hostname, %s", m_aFavoriteServers[m_FavLookup.m_FavoriteIndex].m_aHostname);
					}
					RemoveFavoriteEntry(m_FavLookup.m_FavoriteIndex);
					dbg_msg("serverbrowser", "fav removed multiple entry");
				}
				else
				{
					...
					{
						RemoveFavoriteEntry(m_FavLookup.m_FavoriteIndex);
						dbg_msg("serverbrowser", "fav removed entry that failed hostname-address check");
					}
				}
			}
			else
			{
				...
				if(m_aFavoriteServers[m_FavLookup.m_FavoriteIndex].m_State == FAVSTATE_LOOKUP)
				{
					...
					dbg_msg("serverbrowser", "fav invalid, %s", m_aFavoriteServers[m_FavLookup.m_FavoriteIndex].m_aHostname);
				}
				else
				{
					RemoveFavoriteEntry(m_FavLookup.m_FavoriteIndex);
					dbg_msg("serverbrowser", "fav removed invalid check-based entry");
				}
			}
		}
		m_FavLookup.m_Active = false;
